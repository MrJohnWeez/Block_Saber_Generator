...
# cleanup old map
execute run kill @e[type=marker,tag=blocksaber]
kill @e[type=armor_stand,tag=blocksaber]

# generate new map
summon marker ~ ~ ~ {Tags:[origin,blocksaber]}
...
execute at @e[type=marker,tag=origin,limit=1] run function _root_class:stop
